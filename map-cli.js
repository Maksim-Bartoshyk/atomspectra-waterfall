...
function convertFiles(filepath) {
  ...
  const range8k = sp.rangeToOriginalRange(range, 8);
  const compareRange8k = compareRange ? sp.rangeToOriginalRange(compareRange, 8) : undefined;
  const data = rc.getRctrkData(trackName, deltas, range8k, compareRange8k);
