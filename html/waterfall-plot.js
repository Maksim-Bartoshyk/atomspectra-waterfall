...
constcanvas=document.getElementById('preview-plot');
canvas.width=waterfallData.baseSpectrum.channelCount;
canvas.height=constants.previewHeight;
constctx=canvas.getContext("2d",{willReadFrequently:true});
ctx.fillStyle='#444';
...
letcps=channelValue/combinedSpectrum.duration;
//TODO:duplicatedcode
if(waterfallState.subtractBase&&waterfallData.baseSpectrum.duration>0){
constbaseCps=waterfallData.baseSpectrum.channels[channelIndex]/waterfallData.baseSpectrum.duration;
cps-=baseCps;
if(cps<0){
cps=0;
}
...
//spectrumrender
constspectrumHeight=canvas.height;//-constants.channelAxisHeight;
constimageData=ctx.getImageData(0,0,waterfallData.baseSpectrum.channelCount,spectrumHeight);
constlineColor=hexToRGB('#ABABAB');
cpsInChannel.forEach((cps,channelIndex)=>{
constcounts=Math.round(cps*combinedSpectrum.duration);
constmaxCounts=Math.round(maxCps*combinedSpectrum.duration);
letbarHeight=Math.floor((counts/maxCounts)*spectrumHeight);
...
constfillColor=getPointColor(cps,maxCps);
for(lety=spectrumHeight-barHeight;y<=spectrumHeight;y++){
constcolor=y===spectrumHeight-barHeight
?lineColor
:fillColor;
constpxOffset=(y*waterfallData.baseSpectrum.channelCount+channelIndex)*4;
...
functionrenderWaterfallImage(){
constcanvas=document.getElementById('waterfall-plot');
...
letcps=channelValue/delta.duration;
//TODO:duplicatedcode
if(waterfallState.subtractBase&&waterfallData.baseSpectrum.duration>0){
constbaseCps=waterfallData.baseSpectrum.channels[channelIndex]/waterfallData.baseSpectrum.duration;
cps-=baseCps;
...
constimageData=ctx.getImageData(constants.timeAxisWidth,0,waterfallData.baseSpectrum.channelCount,waterfallData.deltas.length);
waterfallData.deltas.forEach((delta,deltaIndex)=>{
delta.channels.forEach((channelValue,channelIndex)=>{
if(!channelValue){
return;
...
constrgbColor=getPointColor(cps,maxCps);
constpxOffset=(deltaIndex*waterfallData.baseSpectrum.channelCount+channelIndex)*4;
...
constblurData=ctx.createImageData(imageData.width,imageData.height);
constradius=constants.blurRadius;
for(letx=0;x<imageData.width;x++){
for(lety=0;y<=imageData.height;y++){
letr=0;
letg=0;
letb=0;
for(letky=-radius;ky<=radius;++ky){
for(letkx=-radius;kx<=radius;++kx){
constsourcePxOffset=((y+ky)*imageData.width+(x+kx))*4;
...
constdestPxOffset=(y*blurData.width+x)*4;
constcoeff=Math.pow(radius+1,2)*2.1;
...
functiongetPointColor(cps,maxCps){
constlowerCpsBound=maxCps*waterfallState.minCpsPercent/100;
constupperCpsBound=maxCps*waterfallState.maxCpsPercent/100;
cps-=lowerCpsBound;
if(cps<0){
cps=0;
}

constratio=cps/(upperCpsBound-lowerCpsBound);
...
letcolorIndex=Math.round(ratio*(palette.length-1));
switch(waterfallState.scale){
case'log':
...
returnpalette[colorIndex]||palette[palette.length-1];
}

//TODO:temporaryfunctiontodisplaychannelrangeselection
//re-renderschannelaxis
functionrenderChannelSelection(){
constcanvas=document.getElementById('waterfall-plot');
constctx=canvas.getContext("2d",{willReadFrequently:true});
...
//TODO:temporaryfunctiontodisplayspectrumrangeselection
//re-renderstimeaxis
functionrenderSpectrumSelection(){
constcanvas=document.getElementById('waterfall-plot');
constctx=canvas.getContext("2d",{willReadFrequently:true});
...
functionrenderTimeAxis(ctx){
ctx.fillStyle=constants.backgroundColor;
ctx.fillRect(0,0,constants.timeAxisWidth,waterfallData.deltas.length);

consttimestamps=waterfallData.deltas.map(d=>d.timestamp);
for(lettsIndex=0;tsIndex<timestamps.length;tsIndex+=constants.timestampHeight){
consttimestamp=timestamps[tsIndex];
ctx.textBaseline='top';
ctx.fillStyle=constants.textColor;
letlabel=common.timeToString(timestamp);
label+=':'+tsIndex*waterfallState.spectrumBinning*originalWaterfallData.spectrumBinning;
ctx.fillText(label,0,tsIndex);

//labeltick
consttickWidth=tsIndex%100===0
?constants.timestampTickWidth
:constants.timestampTickWidth/2;
for(letx=constants.timeAxisWidth-tickWidth;x<constants.timeAxisWidth;x++){
...
constfromY=Math.floor(waterfallState.spectrumRange[0]/waterfallState.spectrumBinning);
consttoY=Math.floor(waterfallState.spectrumRange[1]/waterfallState.spectrumBinning);
for(lety=fromY;y<=toY;y++){
ctx.fillRect(constants.timeAxisWidth-1,y,1,1);
}
}
}

functionrenderEnergyAxis(canvas,ctx,baseline){
ctx.fillStyle=constants.backgroundColor;
ctx.fillRect(0,waterfallData.deltas.length,canvas.width,constants.channelAxisHeight);

//calculateenergyforeachchannel
constallEnergies=[];
for(leti=0;i<waterfallData.baseSpectrum.channelCount;i++){
constenergy=common.channelToEnergy(i);
allEnergies.push(energy);
}

//calculatechannelsfor0,100,200,300...enegries
constrenderEnergies={};
for(letenergy=0,channel=0;energy<allEnergies[allEnergies.length-1];energy+=100){
...
//energyaxisrender
constenergyAxisBaseline=baseline;
constchannelAxisBaseline=baseline+constants.channelAxisHeight/2;
letkevRendered=false;
for(letx=constants.timeAxisWidth;x<canvas.width;x++){
constchannelNumber=x-constants.timeAxisWidth;
constenergy=renderEnergies[channelNumber];

if(energy!=undefined&&energy%500===0){
ctx.textBaseline='top';
ctx.fillStyle=constants.textColor;
constlabel=kevRendered?energy.toString():energy+'keV';
ctx.fillText(label,x,energyAxisBaseline+constants.channelAxisTickHeight);
kevRendered=true;
}

if(energy!=undefined&&energy%100===0){
consttickHeight=energy%500===0?constants.channelAxisTickHeight:constants.channelAxisTickHeight/2;
for(lety=energyAxisBaseline;y<energyAxisBaseline+tickHeight;y++){
...
ctx.textBaseline='top';
ctx.fillStyle=constants.textColor;
constlabel=channelNumber===0?channelNumber+'channel':channelNumber.toString();
ctx.fillText(label,x,channelAxisBaseline+constants.channelAxisTickHeight);
}

if(channelNumber%10===0){
consttickHeight=channelNumber%50===0?constants.channelAxisTickHeight:constants.channelAxisTickHeight/2;
for(lety=channelAxisBaseline;y<channelAxisBaseline+tickHeight;y++){
ctx.fillRect(x,y,1,1);
}
}
}
...
constfromX=waterfallState.channelRange1[0]+constants.timeAxisWidth;
consttoX=waterfallState.channelRange1[1]+constants.timeAxisWidth;
for(letx=fromX;x<=toX;x++){
...
constfromX=waterfallState.channelRange2[0]+constants.timeAxisWidth;
consttoX=waterfallState.channelRange2[1]+constants.timeAxisWidth;
for(letx=fromX;x<=toX;x++){
