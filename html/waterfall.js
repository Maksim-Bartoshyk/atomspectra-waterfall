(function(){
    ...
    const ironPaletteRGBLin = ironPaletteHex.map(c => hexToRGB(c));
    const ironPaletteRGBSqrt = ironPaletteRGBLin.slice(10);
    const ironPaletteRGBLog = ironPaletteRGBLin.slice(40);
    ...
    function getPointColor(cps, maxCps) {
        ...
        switch (waterfallState.scale) {
            case 'log':
                linColorIndex = Math.round(ratio * (ironPaletteRGBLog.length - 1));
                const logColorIndex = Math.round((Math.log(linColorIndex + 1) / Math.log(ironPaletteRGBLog.length)) * (ironPaletteRGBLog.length - 1));

                return ironPaletteRGBLog[logColorIndex] || ironPaletteRGBLog[ironPaletteRGBLog.length - 1];
            case 'sqrt':
                linColorIndex = Math.round(ratio * (ironPaletteRGBSqrt.length - 1));
                const sqrtColorIndex = Math.round((Math.sqrt(linColorIndex) / Math.sqrt(ironPaletteRGBSqrt.length)) * (ironPaletteRGBSqrt.length - 1));
                
                return ironPaletteRGBSqrt[sqrtColorIndex] || ironPaletteRGBSqrt[ironPaletteRGBSqrt.length - 1];
            default:
                linColorIndex = Math.round(ratio * (ironPaletteRGBLin.length - 1));

                return ironPaletteRGBLin[linColorIndex] || ironPaletteRGBLin[ironPaletteRGBLin.length - 1];
        }
    }
